...
OpenCode plugin that shadows the built-in `webfetch` and `websearch` tools.
It also includes a FastMCP wrapper for the same logic.
...
> [!WARNING]
> Using `git+` with local git repositories (e.g., `git+file://`, local `git+ssh://`) is NOT supported and will not work.
> Always use the `file://` directive for local development.
...
In debug mode the same behavior is exposed as `webfetch_debug`.
...
- **ArXiv**: Routes `arxiv.org` URLs through a local artifact library.
  The library stores PDFs, source archives, BibTeX, and markdown conversions.
...
Supports pagination via `offset` and `num_results`. In debug mode the same behavior is exposed as `websearch_debug`.
