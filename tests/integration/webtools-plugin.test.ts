...
describe("improved-webtools live e2e", () => {
  it(
    "proves webfetch executes via the plugin and returns real content from example.com",
    async () => {
  ...
  it(
    "proves repeated webfetch calls reuse the cached result on the second call",
    async () => {
      await withFixtureServer(
        {
          "/cache": (count) => ({
  ...
  it(
    "proves pdf webfetch returns the temp-download contract instead of a generic binary notice",
    async () => {
  ...
  it(
    "proves overwrite_cache forces a fresh fetch after a cached response exists",
    async () => {
      await withFixtureServer(
        {
          "/refresh": (count) => ({
